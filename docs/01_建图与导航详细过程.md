...
③ 回环候选(每帧取topk=4):
     for i: 选 j>i+gap(40) 且 S[i,j]>sim(0.70) 且 ‖C_i-C_j‖<dmax(4m)
```
> ⚠️ **dmax 必须收紧(默认 4m)**:4路环视会让**矩形房间对边**(相距 ~8m 但互相可见)交叉相似度高;`dmax=12` 会把对边误判为回环、PGO 拉合对边 → **房间挤成楔形**。`dmax=4` 排除对边(8m)、留真回环(同位置 ~2.8m)。
...
原始轨迹 ─┬─ compute_4cam_desc → desc4(626,4,D)
          └─ pgo_fix:  4路交叉S矩阵 → 回环(sim>.7,dmax<4) → SE(2)优化 → z=1.3
                          │
...
| 右上房间被PGO挤扁 | 矩形回路压成楔形 | 4路把**矩形对边**(互见~8m)误判回环、拉合 | 回环**dmax 12→4** 排除对边 |
...
*对应代码版本:GitHub `jx1100370217/VGP-Nav` main(4路PGO/dmax=4、grid_robust(6)、4环视web)。*
