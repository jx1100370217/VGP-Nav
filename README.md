...
- **floor28(创富28楼)**:室内办公单程,真回环闭合残余 0.76m。开放办公区工位高度重复→**强感知混叠**;
  ...
  改用**数据集特定严格检索**(best 为锚 + 半径 3m + 5 refs)后,定位中位 **5.40→2.25m**(相对 6.0%→2.5%)。
...
**floor28 一般**(强混叠经检索优化后相对 2.51%)。两个创富数据集都靠**数据集特定严格检索**改善:
floor28(强混叠)定位中位 5.40→2.25m、floor1(稀疏 DB)12.61→6.14m。
...
- **室内强混叠**:开放办公区工位高度重复 → 检索易把远处相似工位选作参考(floor28)。数据集特定严格检索
  (best 锚 + 小半径 + 少 refs)把定位中位 5.40→2.25m,但更极端混叠仍需更稠密重建/主动传感。
