...
- **floor1(创富1楼)**:户外两街区,IMU 步频 PDR 把 VGGT 偏小的水平尺度校正到 ~470m 真实尺度。
  DB 稀疏(间距 12.8m),检索半径适配到 15m(>间距)后定位中位 12.61→6.14m(相对 0.96%);
  少数重复街角仍有 VPR top1 误匹配(单目室外固有)。
- **floor28(创富28楼)**:室内办公单程,真回环闭合残余 0.76m。开放办公区工位高度重复→**强感知混叠**;
  诊断发现 VPR top1 仍准(10/10 命中真值<3m),误差源于检索多样性把远处相似工位选作参考、污染运动平均。
  改用**数据集特定严格检索**(best 为锚 + 半径 3m + 5 refs)+ **密 DB**(170 DB、query held-out)后,
  定位中位 **5.40→1.09m**(相对 6.0%→1.2%, 良好)。密 DB 让检索到的最近参考更贴 query, 突破 DB 间距对定位精度的限制。

## 建图质量评估 (`scripts/assess_quality.py`)

一套覆盖建图全链路的量化指标 + good·warn·bad 评分,落在可视化「质量评估」tab,也可命令行跑:

```bash
$PY scripts/assess_quality.py        # 遍历全部数据集, 写 outputs/<ds>/quality.json
```

五类指标:

- **A 覆盖度**:DB 节点数 / 轨迹长度 / 地图范围 / 平均 DB 间距 / 点云数。
- **B 轨迹一致性**:相邻帧位移中位与 p95(建图错位会拉大突跳比)、尺度符号(负=重力标反)。
- **C 回环闭合**:视觉相似(4 路交叉描述子)的远时序帧对的建图空间距离(同地点应闭合 <2m)。
- **D 定位精度**:平移误差中位、**相对误差(误差/场景对角线,跨尺度可比)**、<1m/<2m 比例、旋转中位。
- **E 占据连通**:free/占据/未知 占比、最大连通区占比、DB 节点可达率。

三数据集评分:**C8 良好**(定位相对 0.36%、回环 0.21m)、**floor1 良好**(稀疏 DB 经检索优化后相对 0.96%)、
**floor28 良好**(强混叠经严格检索+密DB后相对 1.23%)。三数据集都靠**数据集特定参数**改善定位中位:
floor28(强混叠)严格检索+密DB 5.40→1.09m、floor1(稀疏 DB)检索半径适配 12.61→6.14m。

## 交互网页 (灰度占据栅格 + 动态导航 + Flask 总应用)

总入口 `http://192.168.50.72:8199/`(Flask `scripts/server.py`),**顶层四 tab + 每 tab 内切换地图**:
...
4. **质量评估**:A-E 五类指标卡片 + good·warn·bad 评分 chips + 总评色条;▶运行评估 实时重算 quality.json。
...
3. **检索半径门控**:论文"几何中位+95分位"离群剔除在强感知混叠/稀疏 DB 下失效;改为"以 VPR best 为锚
   + 半径门控",并按数据集在 `datasets.py` 调参:floor28(强混叠)半径 3m / 5 refs、floor1(稀疏 DB)半径 15m / 8 refs。
...
- **室内强混叠**:开放办公区工位高度重复 → 检索易把远处相似工位选作参考(floor28)。数据集特定严格检索
  (best 锚 + 小半径 + 少 refs + 密 DB)把定位中位 5.40→1.09m(良好),但少数帧 VPR top1 反向误匹配(工位镜像)仍错,需更稠密重建/主动传感。
